...
def analyze_github_repo(url: str, tool_context: ToolContext) -> dict[str, Any]:
    ...
    Args:
        url: The GitHub repo URL (e.g., https://github.com/owner/repo)
    ...
    try:
        ...
        if result.get("exit_code", 0) != 0:
            return {
                "ok": False,
                "error": "clone_failed",
                "message": f"Failed to clone: {result.get('output', '')}",
            }
        ...
        lang_cmd = (
            ...
            "-o -name '*.cs' -o -name '*.go' -o -name '*.rs' "
            "-o -name '*.rb' -o -name '*.php' | wc -l"
        )
        lang_result = terminal(command=lang_cmd, workdir=tmpdir)
        analysis["file_count_by_extension"] = (
            lang_result.get("output", "0").strip()
        )

        # Total lines of code (approximate)
        loc_cmd = (
            "find . -type f -not -path './.git/*' "
            "-not -name '*.lock' -not -name '*.png' "
            "-not -name '*.jpg' -not -name '*.gif' "
            "-not -name '*.pdf' -not -name '*.zip' "
        ...
        readme_cmd = (
            ...
            "\\( -iname 'readme*' -o -iname 'readme*.md' "
            "-o -iname 'readme*.txt' -o -iname 'readme*.rst' \\) "
            "| head -1"
        )
        ...
        if readme_path:
            ...
            readme_content = terminal(
                command=readme_content_cmd, workdir=tmpdir
            )
            analysis["readme_preview"] = (
                readme_content.get("output", "")[:500]
            )
        else:
            analysis["readme_preview"] = ""
